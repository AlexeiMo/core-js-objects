...
function mergeObjects(objects) {
  ...
  let resultObj = {};
  resultObj = Object.assign(resultObj, objectsCopy[0]);
  for (let i = 1; i < objectsCopy.length; i += 1) {
    const entries = Object.entries(objectsCopy[i]);
    for (let j = 0; j < entries.length; j += 1) {
      const key = entries[j][0];
      if (Object.keys(resultObj).includes(key)) {
        objectsCopy[i][key] += resultObj[key];
      }
    }
    resultObj = Object.assign(resultObj, objectsCopy[i]);
  }
  return resultObj;
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    let sumToChange = queue[i];
    if (sumToChange !== 25) {
      if (changeArray.length === 0) {
        result = false;
        break;
      }
      while (changeArray.length !== 0 && sumToChange !== 25) {
        sumToChange -= changeArray.shift();
      }
      if (sumToChange !== 25) {
        result = false;
        break;
...
function group(array, keySelector, valueSelector) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    if (!result.has(keys[i])) {
      ...
      const oldValue = result.get(keys[i]);
      oldValue.push(values[i]);
      result.set(keys[i], oldValue);
    }
  }
